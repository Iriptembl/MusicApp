...
function displayEditForm(id) {
    ...
    document.getElementById('edit-id').value = genre.genreId;
...
function updateGenre() {
    ...
    // Check if the genre name already exists
    if (genres.some(genre => genre.genreName === genreName && genre.genreId !== parseInt(genreId, 10))) {
        window.alert('Genre name already exists.');
        return;
    }
...
function _displayGenres(data) {
    ...
    data.forEach(genre => {
        ...
        editButton.setAttribute('onclick', `displayEditForm(${genre.genreId})`);

        let deleteButton = button.cloneNode(false);
        deleteButton.innerText = 'Delete';
        deleteButton.classList.add('delete-button');

        deleteButton.setAttribute('onclick', `deleteGenre(${genre.genreId})`);

        let tr = tBody.insertRow();

        let td0 = tr.insertCell(0);
        let textNodeId = document.createTextNode(genre.genreId);
